...
def mostrar_graficas_costes_ejecucion(datos):
    ...
    if "Coste (€)" in df_plan.columns:
        ...
        df_plan["Coste capítulo"] = df_plan["Peso (%)"] / 100 * coste_total
    else:
        st.error("No se puede calcular el coste por capítulo: faltan columnas 'Coste (€)' o 'Peso (%)'.")
        return

    df_plan["Coste mensual"] = df_plan["Coste capítulo"] / df_plan["Duración (meses)"]
    ...
    for _, row in df_plan.iterrows():
        ...
        coste_mensual = float(-row["Coste mensual"])  # Conversión explícita a float
    ...
    # Calcular "Peso (%)" si solo existe "Coste (€)"
    if "Peso (%)" not in df_plan.columns and "Coste (€)" in df_plan.columns:
