...
def calcular_tir_promotora(datos):
    ...
    try:
        ...
        df_suelo = df_ingresos[["Coste suelo"]].copy() if "Coste suelo" in df_ingresos.columns else pd.DataFrame()

        df_comb = df_ingresos[["Ingresos"]].join(df_necesidades[["Total necesidades"]], how="outer")
        if not df_suelo.empty:
            df_comb = df_comb.join(df_suelo, how="outer")
        df_comb = df_comb.fillna(0)

        df_comb["Flujo neto"] = df_comb["Ingresos"] + df_comb["Total necesidades"] + df_comb.get("Coste suelo", 0)
